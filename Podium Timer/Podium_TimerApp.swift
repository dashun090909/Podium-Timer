...
class AppState: ObservableObject {
    ...
    var protectedTimes: [Double] {
        eventPresets[currentEvent]?.protectedTime ?? [0]
    }

    /// Returns protected minutes per side for a given speech index.
    /// If the event's protectedTime array is exactly [0], protected time is disabled.
    /// If the index is out of range, defaults to 0.
    func protectedMinutesForSpeech(at index: Int) -> Double {
        let arr = protectedTimes
        if arr.count == 1 && arr.first == 0 { return 0 }
    ...
    func resetTimers() {
        ...
        // Configure per-speech protected-time windows
        for (i, timer) in self.timers.enumerated() {
            let minutes = protectedMinutesForSpeech(at: i)
            timer.configureProtectedTime(minutesPerSide: minutes)
        }
